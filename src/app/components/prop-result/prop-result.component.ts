...
export class PropResultComponent implements OnInit {
  ...
  delete(id: number, propietyState: number, e: Event) {
    e.stopPropagation();
  ...
  activateProp(id: number, e: Event): void {
    e.stopPropagation();

    this.propiedadesService.activate(id)
      .subscribe(res => {
        this.pedirBusqueda(true);
        this.propiedadesService.getCountsPropiedades(this.propietario)
          .subscribe(res => {
  ...
  activateAllProp(): void {
    let request = this.propiedades
      .filter(prop => prop.isSelected)
      .map(prop => {
        return this.propiedadesService.activate(+prop.id);
      });
  ...
  toggleState(e: Event, propiedad) {
    e.stopPropagation();
    propiedad.isSelected = !propiedad.isSelected;
  }

  redirectToFicha(id: number) {
